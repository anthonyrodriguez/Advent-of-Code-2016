encrypted = """hqcfqwydw-fbqijys-whqii-huiuqhsx-660[qhiwf]
...
import itertools
encryptedList = encrypted.splitlines()
sectorIDSum = 0
for room in encryptedList:
    letterCount = {}
    splitName = room.split("-")
    score = (int)(splitName[-1].split("[")[0])
    checksum = splitName[-1][-6:-1]
    for letter in list(itertools.chain.from_iterable(splitName[:-1])):
    ...
    for letter in letterCount:
        if letterCount[letter] in occurrenceList:
            occurrenceList[letterCount[letter]].append(letter)
        else:
            occurrenceList[letterCount[letter]] = [letter]
    finalLetterList = []
    for occurrence in sorted(occurrenceList, reverse=True):
        finalLetterList += sorted(occurrenceList[occurrence])
    if ''.join(finalLetterList[:5]) == checksum:
            sectorIDSum += score
print sectorIDSum 
