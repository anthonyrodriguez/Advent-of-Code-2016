ipAddresses = """dnwtsgywerfamfv[gwrhdujbiowtcirq]bjbhmuxdcasenlctwgh
...
def getABA(word):
    ...
    for i in xrange(len(word)-2):
        if word[i] != word[i+1] and word[i] != '-' and word[i+1] != '-':
            if word[i] == word[i+2]:
                toReturn.append(word[i:i+3])
    return toReturn

ipList = ipAddresses.splitlines()
correctIPs = 0
for ip in ipList:
    outsideBrac = ''
    insideBrac = ''
    ...
    for letter in ip:
        if not inBrac:
            ...
            if letter == ']':
                ...
                insideBrac += letter
    for aba in getABA(outsideBrac):
        bab = aba[1]+aba[0]+aba[1]
        if bab in insideBrac:
            correctIPs += 1
            break
print correctIPs
