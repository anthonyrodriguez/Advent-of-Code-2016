bots = {}
outputs = {}
#botNumber: [low, high, chip1, chip2]
# for low and high, neg number indicates output
"""instructionString = value 5 goes to bot 2
bot 2 gives low to bot 1 and high to bot 0
value 3 goes to bot 1
bot 1 gives low to output 1 and high to bot 0
bot 0 gives low to output 2 and high to output 0
value 2 goes to bot 2"""
instructionString = """value 61 goes to bot 209
...
instructions = instructionString.splitlines()
chipsOut = 0
for instruction in instructions:
    if instruction.startswith('value'):
        botNum = (int)(instruction.split(' ')[-1])
        newVal = (int)(instruction.split(' ')[1])
        if botNum not in bots:
            bots[botNum] = [-1, -1]
        if len(bots[botNum]) == 2:
            bots[botNum].append(newVal)
        elif len(bots[botNum]) == 3:
            oldVal = bots[botNum][2]
            if newVal > oldVal:
                bots[botNum].append(newVal)
            else:
                bots[botNum][2] = newVal
                bots[botNum].append(oldVal)
        else:
            print "BAD ERROR INSTRUCTIONS WRONG IDK"
        chipsOut += 1
    else:
        ...
        if split[-2] == 'output':
            high = -1-high
        if botNum not in bots:
            bots[botNum] = [low, high]
        else:
            bots[botNum][0] = low
            bots[botNum][1] = high

botWithFour = True
while chipsOut > 0 and botWithFour:
    botWithFour = False
    for botNum in bots:
        botList = bots[botNum]
        if len(botList) == 4:
            if botList[2] == 17 and botList[3] == 61:
                print botNum
            botWithFour = True
            if botList[0] < 0:
                outputs[-1-botList[0]] = botList[2]
                chipsOut -= 1
            else:
                if len(bots[botList[0]]) == 2:
                    bots[botList[0]].append(botList[2])
                elif len(bots[botList[0]]) == 3:
                    oldVal = bots[botList[0]][2]
                    newVal = botList[2]
                    if newVal > oldVal:
                        bots[botList[0]].append(newVal)
                    else:
                        bots[botList[0]][2] = newVal
                        bots[botList[0]].append(oldVal)
                else:
                    print "BAD ERROR BOTS WRONG IDK"
            if botList[1] < 0:
                outputs[-1-botList[1]] = botList[3]
                chipsOut -= 1
            else:
                if len(bots[botList[1]]) == 2:
                    bots[botList[1]].append(botList[3])
                elif len(bots[botList[1]]) == 3:
                    oldVal = bots[botList[1]][2]
                    newVal = botList[3]
                    if newVal > oldVal:
                        bots[botList[1]].append(newVal)
                    else:
                        bots[botList[1]][2] = newVal
                        bots[botList[1]].append(oldVal)
                else:
                    print "BAD ERROR BOTS WRONG IDK"
            botList.pop()
            botList.pop()
